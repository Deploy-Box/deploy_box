...
class RedisStackManager(StackManager):
    @staticmethod
    def get_purchasable_stack_info():
        return {
            ...
            "variant": "STARTER",
            "version": "1.0",
            "description": "A Redis in-memory data store deployed on Azure Virtual Machine.",
            "price_id": "TEST",
            "features": [
                "Azure Virtual Machine hosting Redis server",
                "Pre-configured Redis environment for quick deployment",
    ...
    def get_starter_stack_iac_attributes(self):
        ...
        return {
            ...
            "azurerm_public_ip.azurerm_public_ip-1.name": f"redis-vm-ip-{self.stack.id}",
            ...
            "azurerm_network_interface.azurerm_network_interface-1.name": f"redis-vm-nic-{self.stack.id}",
            ...
            "azurerm_linux_virtual_machine.azurerm_linux_virtual_machine-1.computer_name": f"redis-vm-{self.stack.id}",
            ...
            "azurerm_linux_virtual_machine.azurerm_linux_virtual_machine-1.name": f"redis-vm-{self.stack.id}",
            ...
            "azurerm_linux_virtual_machine.azurerm_linux_virtual_machine-1.os_disk.name": f"redis-vm-osdisk-{self.stack.id}",
