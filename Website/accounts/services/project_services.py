    ...
    if request.method == "POST":
        try:
            ...
            if not check_project:
                ...
                Project.objects.create(name=name, description=description, organization_id=org_id)
        ...
        try:
            ...
            if project:
                ...
                if "organization_id" in data:
                    ...
                    if org:
                        project.organization = data.get("organization_id", project.organization)
                    else:
        ...
        try:
            ...
            if project:
                if not project_member:
                    ...
                    return JsonResponse({"message": "project member already exists"}, status=404)
            else:
                return JsonResponse({"message": "project does not exist"}, status=404)
        except Exception as e:
            return JsonResponse({"error": f"An unexpected error occurred: {str(e)}"}, status=500)
...
def delete_project_member(request: HttpRequest) -> JsonResponse:
    if request.method == "POST":
        try:
            ...
            project_member = ProjectMember.objects.filter(user_id=user_id, project_id=project_id).first()

            if project_member:
                project_member.delete()

                return JsonResponse({"message": "user has been removed from the project"})
            else:
                return JsonResponse({"message": "user is not associated with this project"})
        except Exception as e:
